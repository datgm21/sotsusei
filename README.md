# 卒業制作情報共有用リポジトリ(2021年度入学生)

## 基本情報
- [Slack](https://datgm21.slack.com)
- [質問・報告](https://meet.google.com/gcu-fdoy-ysh)
- [参考URL](https://github.com/datgm21/sotsusei/blob/main/URLs.md)
- [講義日程](https://github.com/datgm22/gp1/blob/main/thus.md)


## 参考資料
- [Unityの命名規則](http://am1tanaka.hatenablog.com/entry/2019/12/06/101055)
- [Unity用のFBXファイルをMayaで出力する](https://docs.google.com/document/d/1X-Ew4g2jvXaGmxpctjim81StWgCAIz31DQqB_Y3IcyU/)
- [Unityで使用する画像ファイル](https://docs.google.com/document/d/1KxhV1sL614Ui90LyaXOegaEkIFTg6c6eTam_ntYZl34/)
- [グラフィックデータのまとめ方](https://docs.google.com/document/d/1I30TPQtOu5n3PwTNdGhL-SZ-ddVt98Ifira_Wp5n06U/)
- [2020年度のノベル作品の例](https://github.com/datgm20/sotsusei/wiki/%E3%83%8E%E3%83%99%E3%83%AB%E3%82%B5%E3%83%B3%E3%83%97%E3%83%AB)
- [曲のビートのタイミングを計算する方法](https://am1tanaka.hatenablog.com/entry/beat-calc#%E6%9B%B2%E3%81%AE%E3%83%86%E3%83%B3%E3%83%9D%E3%82%92%E6%8A%8A%E6%8F%A1%E3%81%99%E3%82%8B)
- [2Dプラットフォーマー用プレイヤーシンプル版](https://docs.google.com/presentation/d/1JT3qWDmEP1DzptlampgO8s6G98_xDo0vjBz7Gn-OI1I/)


## 年間予定

|期間|内容|
|:-|:-|
|4/21-5/12|企画のアイディア出し、画面イメージ作り|
|5/13(金)|企画＆チーム決定|
|7/8(金)|試遊会|
|7/22(金)|試遊会(2)|
|7/25-8/28|-夏休み-|
|9/9(金)|夏休み成果発表|
|<s>10/20(木)</s>|<s>試遊会</s> -> 21日へ移動|
|10/21(金)|試遊会(βテスト) + プレゼン</s>|
|<s>11/4(金)</s>|<s>プレゼン</s> -> 21日へ移動|
|11/11(金), 12(土)|DATフェスタ|
|12月末|試遊会|
|2月|Web公開とDVD作成|
|2/3(金)|最終日|

## 後期15回目(2/2)

### 資料
- [DVDの焼き方](https://github.com/datgm20/sotsusei/blob/main/dvd_write.md)
- [破壊オブジェクトを作成するパッケージ](https://github.com/tanakaedu/MeshBreaker)

### 予定
- レポートレビュー
- DVDのラベルとパッケージデザイン
  - `X:\2022年\ゲーム学科2\平井・田中・細谷\DVD`に各チームごとのパッケージデザインが提出されるので、代表作を選んで印刷(明日？)
- 作業
  - 無音部分のカットなど
    - インストール
      - https://github.com/audacity/audacity/releases を開く
      - 最新版の `audacity-win-x.x.x-x64.zip` をクリックしてダウンロード
      - ドキュメントの自分のフォルダー内などに移動して、右クリック > 7-Zip > audacity-win-x.x.x-x64に展開 を選択
      - zipは削除してよい
      - 展開したフォルダー内のAudacity.exeを起動
      - aup3ファイルの関連付けは はい を選択
    - 編集
      - 起動したら、ウィンドウに編集したい音声ファイルをドラッグ&ドロップで読み込む
      - Ctrl + 1 キーで波形を拡大して見やすくする。縮小は Ctrl + 3 キー
      - 無音部分をドラッグで選択
      - 編集メニューから切り取りで不要な部分を削除
      - 再生して問題がないことを確認
      - ファイルメニューから書き出しを選択して、MP3かOGGのいずれか使いたい方を選択
      - 保存先のUnityプロジェクトフォルダーを選んで、ファイル名を設定して上書き保存
      - メタデータはそのままでOKをクリック
      - 別の音声を編集するときは、編集が終わった音声は左上の×ボタンをクリックして閉じてから次の音声ファイルをドラッグ&ドロップする
- プロジェクト提出
  - Unityプロジェクトをデスクトップにコピーする
  - デスクトップにコピーしたUnityプロジェクトフォルダーを開いて `.git`と`Library`フォルダーを削除する
  - デスクトップにコピーしたプロジェクトフォルダーを右クリックして 7-Zip > 圧縮を選んでzip形式で圧縮
  - 圧縮したzipファイルを`X:\2022年\ゲーム学科2\平井・田中・細谷\最終プロジェクト`内にコピー
- まとめ

## 後期14週目(1/26)

### 資料
- [DVDの焼き方](https://github.com/datgm20/sotsusei/blob/main/dvd_write.md)
- [破壊オブジェクトを作成するパッケージ](https://github.com/tanakaedu/MeshBreaker)

### 内容

Webページ、実行ファイルの提出(評価版)。

- Webページ(最終候補)
  - ドキュメントの文章や画像をコミット＞プッシュ＞プルリクエスト
    - Pull requests タブを選択して、Create Pull request を押して編集結果の更新を依頼
  - [2021年度作品ページ](https://datgm21.github.io/works)
    - [作成の手引き](make-web.md)
  - 参考 [2020年度生 卒業制作作品集](https://datgm20.github.io/works/)
- 各チーム、作業
  - [講評](https://docs.google.com/document/d/1Sd5GpL1vxW9WSDW9am8dEbeazo02xUuUWhc85OXGxuY/)
  - [機械戦線](https://docs.google.com/document/d/1dUhNynLK89gguxgYZPJNdjYdhhC-xPLMog5urYEWZG4/edit?usp=sharing)
    - 戦闘メカニクスの検討
    - 木とやぶの差し替え
  - [CAR CRASH. オブジェクトの分割(ビルを破壊する)](https://docs.google.com/document/d/1CYdzVXSr5k7x8BWFoY20FxfbTg3F2y5q6h5duAduxx4/edit?usp=sharing)
    - 分割エディター拡張
  - [Black Holeプレイヤー移動の設計](https://docs.google.com/document/d/1yLhEZd8qyqFe52ZLxApXn2KPtlIrdAaYqwMS9U4gU-A/edit?usp=sharing)
- 実行ファイル(評価版)の更新
  - 評価版として実行ファイルをビルドして `X:\2022年\ゲーム学科2\平井・田中・細谷\最新実行ファイル` 内の各フォルダへ上書きコピーして提出
  - Webページのzipもこれに更新してコミット＞プッシュ＞プルリクエスト


## 後期10-13週目(12/22, 1/5, 12, 19)

### 話題
- 仕様作成演習。チーム制作演習：企画の仕様作成
  - [ゲームクリエイター甲子園. 元カプコンPから学ぶ！ゲーム企画書のつくりかた【入門編】](https://youtu.be/57onvHQyF9s)
  - [ゲームクリエイター甲子園. 元カプコンPから学ぶ！ゲーム企画書のつくりかた【入門編】 企画書のコツ](https://youtu.be/57onvHQyF9s?t=1092)

### 資料
- [学校のロゴデータ](X:\2022年\ゲーム学科2\LOGO)
- [DVDパッケージとラベルのフォーマット](https://github.com/datgm20/sotsusei/blob/main/dvd-format.md)
- [DVDの焼き方](https://github.com/datgm20/sotsusei/blob/main/dvd_write.md)

### 内容
- Webページの更新
  - 動作が安定した実行ファイルがビルドできる状況であれば、ビルドして `X:\2022年\ゲーム学科2\平井・田中・細谷\最新実行ファイル` 内の各フォルダへ提出
  - ドキュメントの文章をコミットしてプッシュ
  - Pull requests タブを選択して、Create Pull request を押して編集結果の更新を依頼。これ以降、変更があったらコミットしておけばよい
  - [2021年度作品ページ](https://datgm21.github.io/works)
    - [作成の手引き](make-web.md)
  - 参考 [2020年度生 卒業制作作品集](https://datgm20.github.io/works/)
- 各チーム、作業
  - [講評](https://docs.google.com/document/d/1Sd5GpL1vxW9WSDW9am8dEbeazo02xUuUWhc85OXGxuY/)
  - [機械戦線](https://docs.google.com/document/d/1dUhNynLK89gguxgYZPJNdjYdhhC-xPLMog5urYEWZG4/edit?usp=sharing)
    - 戦闘メカニクスの検討
    - 木とやぶの差し替え
  - [CAR CRASH. オブジェクトの分割(ビルを破壊する)](https://docs.google.com/document/d/1CYdzVXSr5k7x8BWFoY20FxfbTg3F2y5q6h5duAduxx4/edit?usp=sharing)
    - 分割エディター拡張
  - [Black Holeプレイヤー移動の設計](https://docs.google.com/document/d/1yLhEZd8qyqFe52ZLxApXn2KPtlIrdAaYqwMS9U4gU-A/edit?usp=sharing)


## 後期9週目(12/15)

### 資料
- [学校のロゴデータ](X:\2022年\ゲーム学科2\LOGO)
- [DVDパッケージとラベルのフォーマット](https://github.com/datgm20/sotsusei/blob/main/dvd-format.md)
- [DVDの焼き方](https://github.com/datgm20/sotsusei/blob/main/dvd_write.md)

### 内容
- 各チーム、作業
  - [講評](https://docs.google.com/document/d/1Sd5GpL1vxW9WSDW9am8dEbeazo02xUuUWhc85OXGxuY/)
  - [機械戦線](https://docs.google.com/document/d/1dUhNynLK89gguxgYZPJNdjYdhhC-xPLMog5urYEWZG4/edit?usp=sharing)
    - 戦闘メカニクスの検討
    - 木とやぶの差し替え
  - [CAR CRASH. オブジェクトの分割(ビルを破壊する)](https://docs.google.com/document/d/1CYdzVXSr5k7x8BWFoY20FxfbTg3F2y5q6h5duAduxx4/edit?usp=sharing)
    - 分割エディター拡張
  - [Black Holeプレイヤー移動の設計](https://docs.google.com/document/d/1yLhEZd8qyqFe52ZLxApXn2KPtlIrdAaYqwMS9U4gU-A/edit?usp=sharing)
- 公開用Webページの作成
  - [2021年度作品ページ](https://datgm21.github.io/works)
    - [作成の手引き](make-web.md)
  - 参考 [2020年度生 卒業制作作品集](https://datgm20.github.io/works/)


## 後期6, 7, 8週目(11/24, 12/1, 8)

### 内容
- 各チーム、作業
  - [講評](https://docs.google.com/document/d/1Sd5GpL1vxW9WSDW9am8dEbeazo02xUuUWhc85OXGxuY/)
  - [機械戦線の戦闘メカニクスの検討](https://docs.google.com/document/d/1dUhNynLK89gguxgYZPJNdjYdhhC-xPLMog5urYEWZG4/edit?usp=sharing)
    - カメラの補間アルゴリズム
  - [CAR CRASH. オブジェクトの分割(ビルを破壊する)](https://docs.google.com/document/d/1CYdzVXSr5k7x8BWFoY20FxfbTg3F2y5q6h5duAduxx4/edit?usp=sharing)
  - [Black Holeプレイヤー移動の設計](https://docs.google.com/document/d/1yLhEZd8qyqFe52ZLxApXn2KPtlIrdAaYqwMS9U4gU-A/edit?usp=sharing)
- 公開用Webページの作成
  - [2021年度作品ページ](https://datgm21.github.io/works)
    - [作成の手引き](make-web.md)
  - 参考 [2020年度生 卒業制作作品集](https://datgm20.github.io/works/)


## 後期5週目

### 話題
- [来週水曜日 池袋もくもく会](https://weekend-unity.connpass.com/event/266346/?utm_campaign=event_publish_to_series_watcher&utm_source=notifications&utm_medium=email&utm_content=detail_btn)


### 内容
- [講評続き](https://docs.google.com/document/d/1Sd5GpL1vxW9WSDW9am8dEbeazo02xUuUWhc85OXGxuY/)
  - チーム内のまとめドキュメントを書き終えていたら編集を**閲覧者に変更**
- 作品のまとめ
  - 参考： [昨年度の卒業制作まとめサイト](https://datgm20.github.io/works/)


## 10/21の試遊＆プレゼン会について
- 10/20(木)に予定されていた試遊会と11/4(金)のプレゼンが分かれていると作業時間が取られて勿体ないという理由で、10/21(金)にまとめて実施することに変更
- 10/21の試遊会がDATフェスタ前に他チームのメンバーに見てもらえる最後の機会である。これを逃すとゲームを知らない人が企画の意図に気付けるかや、操作性に問題がないか、伝わりにくいところがどこかなどを知ることができなくなる。この段階でゲームとして必要な要素は全て盛り込んでおく必要がある
- これ以降は原則として、バランス調整、テストとデバッグ、クオリティアップの期間になる。残りの期間から実装するものと見送るものを見極めて、最小限の展示可能バージョンをまとめるための計画を考えること


## 後期4週目(10/27)

### 内容
- [4th試遊会 講評](https://docs.google.com/document/d/1Ig1GIQ3-NuCghojCiXrIHxmczA0OsbEKLTXcaYHUTkQ/)
  - Black Holeの講評
- 残りの日程
  - 27(木), 28(金)
  - 11/2(水), 11/4(金)
  
### DATフェスタ準備
- 11/9(水) 実行ファイルの設定、屋内装飾、全ての作品の操作の確認
- 11/10(木) 作業が残っていた場合の予備日



## 後期3週目(10/20)

### 内容
- 進捗と残り作業の確認
  - A マップ、車の仕上げ、調整
    - 車の仕上げ完了(スクリプトの対応ミス修正、モデルのタイヤが軸中心になっていなかったので、ProBuilderで中心座標を修正して、スクリプトを修正)
  - B 保健室や教室の実装中。シナリオに沿って実装を進める
    - 開始からメッセージやセリフを表示したり目的地に移動すると話が進むなど、ゲームを遊ぶのに必要な部分から作る。マップに違う部分があったら指示は伝えつつ、ProBuilderなどで仮のオブジェクトを作成して開発を止めないようにする
  - C クリアやゲームオーバーを実装して、残りは中身だけの状態にもっていく
    - 裏世界の入り口を全て閉じることで一先ずクリアする方向で進める。それができて時間があったらボスの作成に着手。入り口をいくつか塞ぐとカットシーンで敵が異常を感知したようなストーリーを見せたり、新たな入り口が開いたりといったストーリー展開も考えられる。ゲーム本体を早めに完成させて、そのような付加要素を追加できるのが理想
  - D Slackの項目を確認→メモ帳に項目を記載
    - プレイヤーのジャンプなどのアニメを設定 / 敵を攻撃したときのマスク設定の修正

## 後期2週目(10/13)

### 内容
- 進捗と残り作業の確認
  - A マップの仕上げ、クリア条件の実装、調整
  - B 保健室や教室の実装中。シナリオに沿って実装を進める
    - 開始からメッセージやセリフを表示したり目的地に移動すると話が進むなど、ゲームを遊ぶのに必要な部分から作る。マップに違う部分があったら指示は伝えつつ、ProBuilderなどで仮のオブジェクトを作成して開発を止めないようにする
  - C クリアやゲームオーバーを実装して、残りは中身だけの状態にもっていく
    - 裏世界の入り口を全て閉じることで一先ずクリアする方向で進める。それができて時間があったらボスの作成に着手。入り口をいくつか塞ぐとカットシーンで敵が異常を感知したようなストーリーを見せたり、新たな入り口が開いたりといったストーリー展開も考えられる。ゲーム本体を早めに完成させて、そのような付加要素を追加できるのが理想
  - D Slackの項目を確認→メモ帳に項目を記載


## 後期1週目(10/6)

### 内容
- 進捗確認と前期最後からの続き
  - A 街の進捗。街を段差で囲む / 道路と建物の設定 / 車の差し替え
  - B 開始からメッセージやセリフを表示したり目的地に移動すると話が進むなど、ゲームを遊ぶのに必要な部分から作る。マップに違う部分があったら指示は伝えつつ、ProBuilderなどで仮のオブジェクトを作成して開発を止めないようにする
  - C 裏世界の入り口を全て閉じることで一先ずクリアする方向で進める。それができて時間があったらボスの作成に着手。入り口をいくつか塞ぐとカットシーンで敵が異常を感知したようなストーリーを見せたり、新たな入り口が開いたりといったストーリー展開も考えられる。ゲーム本体を早めに完成させて、そのような付加要素を追加できるのが理想
  - D プレイヤーの動かし方の説明と、Rigidbody2Dでの簡単な移動を完了。壁に引っかからないように抵抗の設定と、接触時の向きの確認、着地確認などを実装する
- マップ間ワープの考え方
- キャラクタアニメの方針の確認
- ロボットモーションの相談


---

## 15週目(9/15)
### 内容
- [3rd試遊会版について](https://docs.google.com/document/d/1_j4RQ9Oi3KvPrVwK-Dk6Xtszrd9KerKK5d4SRPzZ3dw/)
- 今後の進行の確認
- 開発

## 14週目(9/8)

### 内容
- 9/9の成果発表向け作業と質疑応答
  - C班のモデルはいったんアセットで進めてもよい
- 最終ビルドは金曜日に行って、共有フォルダー`X:\2022年\ゲーム学科2\平井・田中・細谷\3rd_試遊会`に実行ファイルを置いて共有
  - 発表をする場合は、既知の不具合、未実装の項目を簡潔に伝える 
- 9/15に改めて成果を確認して、DATフェスタ向けの作業を確認する


## 13週目(9/1)

### 話題
- [桜井政博のゲーム作るには. 遊びの仕事は遊びじゃない](https://www.youtube.com/watch?v=5DS_aGjo9zY)

### 内容
- 各自、夏休み成果の報告
  - 一人5分程度。最長10分。最短制限なし
  - 発表順になって前の人が切断したら Google Meet で画面共有
  - 担当していた箇所を画面に映して報告
  - 9/9の成果発表までの作業予定
- 成果報告に向けたチーム打ち合わせにあたって、ゲームCGや企画側に確認する必要があるものを文章にまとめる。時間があればこの時間に打ち合わせるとよい
- 技術的な不明点の質疑応答


## 12週目(7/21)

### 参考
- [濱村 崇 / GameDesignLab. 試作・実験](https://twitter.com/GDLab_Hama/status/1541547329890942976)
- [濱村 崇 / GameDesignLab. 試作・実験(2)](https://twitter.com/GDLab_Hama/status/1542297190986842114)

### 内容
- 試遊会(2)に向けた準備作業と質疑応答
- [作業リスト例](https://docs.google.com/spreadsheets/d/1a81GVve_EQK8OMuRC3rSDjF-qTVIc94dfAAB9KoV1fw/)

## 11週目(7/14)
- 試遊版の確認と今後の作業の確認
- 来週、夏休み明けの成果発表の目標

## 10週目(7/7) 試遊会準備

### 試遊会
- 目的
  - 開発メンバー以外の人達に、企画の狙いや遊び方が伝わっているかを確認する
- 必要なもの
  - ビルドファイル一式
  - 簡単なゲーム概要と操作説明を書いたテキストファイル
    - ビルドフォルダーに`README.txt`というテキストファイルを作成して、そこに記載
    - unityroomの説明参考
  - ESCキーで終了するようにする
- データの提出先
  - `X:\2022年\ゲーム学科2\平井・田中・細谷\1st_試遊会`フォルダー内に作品名のフォルダーを作成して、そこにビルドしたファイル一式とREADME.txtをコピー
- 明日
  - ※ネットドライブから直に起動すると遅いので、試遊時は各自のPCのデスクトップにデータをコピーしてから、コピーしたファイルを起動する
  - 試遊中に、メモ帳などのテキストファイルで、遊んだ感想や気づいた不具合、改善案などを記入する
    - 提出先 `X:\2022年\ゲーム学科2\平井・田中・細谷\1st_試遊会\みんなの感想`の中のグループ名のフォルダー内
    - テキストファイル名は、出席番号と氏名にする(例 `00tanaka.txt` など)

### 今日の目標
- 試遊できるものをビルドして、共有ドライブにコピー
- コピーしたフォルダー内に`README.txt`ファイルを用意

今日中に間に合わなかった場合は明日の試遊会までに準備する。


## 6-9週目(6/9, 16, 23, 30)
- 7/8に向けて作業予定の確認、質疑応答、作業
- 作業項目のリストアップ、開始予定日、完了予定日、担当者、進捗(未着手/作業中/完了)を1行ごとに書いた作業予定リストを作成
  - Googleスプレッドシートで作成して共有を推奨。メモ帳で共有ドライブでも可
  - 参考 [ITトレンド. ガントチャートとは？作り方のコツやおすすめツールを紹介](https://it-trend.jp/project_management/article/33-0008)
    - 右側の期間のセルを塗りつぶす部分は管理が面倒になりがちなので省略してよい


## 5週目(6/2)
- 次の目標 7/8(金) 試遊会
- 企画の進捗確認
  - A601で9:30頃からA班から開始
- コースで分かれて作業

## 4週目(5/19)
- 企画の共有と検討
  - 前回までに企画案を共有していない場合
    - Googleドキュメント、Googleスライド、WordやPowerPointのデータをGoogleドライブにアップしたものなどを共有して、`リンクを知っている全員に変更`をクリックして、リンクをコピー
    - [こちら](https://docs.google.com/forms/d/e/1FAIpQLSez2qy7CH6hRwDInk3qD_dfjoBB2Up8Q9uCMZS1feIVv5r8iQ/viewform?usp=sf_link)にコピーしたドキュメントの共有URLを報告
  - [企画一覧表]から、参加したい、あるいは、参加してもよい、という作品を探す
- 参加希望表への記入
  - 参加したい、あるいは、参加してもよい、という企画の行の参加希望列の空欄に自分の氏名とコースを記入する
    - プログラマーの場合はpg、ゲームCGキャラクターの場合はcgを名前の後ろにつける。例 田中pg　細谷cg
    - 希望表明をする企画の上限はなし
    - 選出する企画は4つ程度の予定
    - 希望する企画が複数選択された場合、メンバー構成や本人の希望からどれか一つをメイン企画としてあとで選ぶ
- チームミーティング
  - Googleドキュメントを作成してURLをメンバーで共有
  - GoogleドキュメントのURLを[こちら](https://docs.google.com/forms/d/e/1FAIpQLSez2qy7CH6hRwDInk3qD_dfjoBB2Up8Q9uCMZS1feIVv5r8iQ/viewform?usp=sf_link)に報告。氏名欄には企画一覧表の企画名を書く
  - 自己紹介
  - 企画について、自分が担当できること、企画について思いついたこと、企画で分からないこと、これから考える必要がありそうなこと、などを話し合い、Googleドキュメントに記入していく

## 3週目(5/12)

### 内容
- 企画の共有と質疑
  - 前回までに企画案を共有していない場合
    - Googleドキュメント、Googleスライド、WordやPowerPointのデータをGoogleドライブにアップしたものなどを共有して、`リンクを知っている全員に変更`をクリックして、リンクをコピー
    - [こちら](https://docs.google.com/forms/d/e/1FAIpQLSez2qy7CH6hRwDInk3qD_dfjoBB2Up8Q9uCMZS1feIVv5r8iQ/viewform?usp=sf_link)にコピーしたドキュメントの共有URLを報告
  - [企画アイディアのまとめ方](https://docs.google.com/document/d/1_DssJw_yhy3QZwinSIvjlfOxMIcr9airKHb5-unula4/edit?usp=sharing)
  - EXの方は金曜日に進めるので、木曜日は卒業制作用のものを進める
  - 検討している企画案は、GoogleスライドかGoogleドキュメントにまとめる
  - [海賊すごろく企画書例](https://github.com/tanakaedu/PirateSugorokuPractice/blob/main/Documents/proposal/index.md)
- 企画のまとめ
  - 金曜日発表用にまとめる
  - 発表は共有URLでドキュメントにアクセス
  - 企画書だけでもよい。可能なら画面のモックアップや試作動画を作成
    - [Voodooの企画](https://youtu.be/ck3yw4xoPNU?t=3494)
    - [Voodooの進行](https://youtu.be/ck3yw4xoPNU?t=3726)

## 2週目(4/28)

### 話題
- [ゲームクリエイター甲子園2022](https://game.creators-guild.com/g4c/16402/)

### 内容
- 企画の報告と相談
  - [こちら](https://docs.google.com/forms/d/e/1FAIpQLSez2qy7CH6hRwDInk3qD_dfjoBB2Up8Q9uCMZS1feIVv5r8iQ/viewform?usp=sf_link)に作成したドキュメントの共有URLを報告
    - Googleドキュメント、Googleスライド、WordやPowerPointのデータをGoogleドライブにアップしたものなどを共有して、`リンクを知っている全員に変更`をクリックして、リンクをコピー
  - 説明と質疑応答
- 企画案を引き続き検討
  - [企画アイディアのまとめ方](https://docs.google.com/document/d/1_DssJw_yhy3QZwinSIvjlfOxMIcr9airKHb5-unula4/edit?usp=sharing)
  - EXの方は金曜日に進めるので、木曜日は卒業制作用のものを進める
  - 金曜日は教室が不明なので、共有ドライブのデータはアクセスできない可能性がある。GoogleドライブやGitHubなどのクラウドサービスを利用する
  - 検討している企画案は、GoogleスライドかGoogleドキュメントにまとめる
  - [海賊すごろく企画書例](https://github.com/tanakaedu/PirateSugorokuPractice/blob/main/Documents/proposal/index.md)
- 企画案をダウンロードして、以下に提出
  - `Z:\2022\student\gp2\00企画案`
  - ドキュメント名(ファイル名)を「氏名_企画案」として、wordか、PDFでダウンロードしたものをコピーして提出


## 1週目(4/21)

- 木曜日：プログラムとCGに分かれて、主に開発を進める
- 金曜日：グループで企画について打ち合わせる

制作は学生主体で行うことを原則とする。企画内容や仕様、進捗を教員に伝え、教員から出た質問を記録してチームの打ち合わせで話し合った結果を次の週に回答する。メモや進め方を工夫して欲しい。

### 今週やること
- 木曜日(今日)：PCの動作確認。各自、卒業制作で作ってみたい作品の企画案を出す
- 金曜日(明日)：企画をクラス内で共有。自分以外の作品で好みのものがあれば、そこに合流。次週の木曜日に、各自、それぞれ何を選んだかを報告

### 話題
- [ゲームクリエイター甲子園2021受賞作品紹介](https://youtu.be/WVOuZTmua_8)

### 下準備
- [企画アイディアのまとめ方](https://docs.google.com/document/d/1_DssJw_yhy3QZwinSIvjlfOxMIcr9airKHb5-unula4/edit?usp=sharing)
  - EXの方は金曜日に進めるので、木曜日は卒業制作用のものを進める
  - 金曜日は教室が不明なので、共有ドライブのデータはアクセスできない可能性がある。GoogleドライブやGitHubなどのクラウドサービスを利用する
  - 検討している企画案は、GoogleスライドかGoogleドキュメントにまとめる
- インストール状況の確認
  - UnityHub3.1.1
  - Unity2019.4.22f1 + Android Build Support + iOS Build Support + Mac Build Support
  - Unity2021.3.0f1 + WebGL
- 企画案をダウンロードして、以下に提出
  - `Z:\2022\student\gp2\0421企画案`
  - ドキュメント名(ファイル名)を「0421氏名_企画案」として、wordか、PDFでダウンロードしたものをコピーして提出


